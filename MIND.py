...
"""
https://github.com/tomosu/MIND-pytorch
"""
...
if __name__ == "__main__":
    ...
    # print(mind(orig).size())

    import numpy as np
    from skimage.io import imread
    from skimage.color import rgb2gray

    f_img = imread("final_data/optical-lidar-3/f_img.tif")
    f_img = rgb2gray(f_img)
    m_img = imread("final_data/optical-lidar-3/m_img.tif")[:, :, 0]
    # m_img = imread("final_data/optical-lidar-3/ssim_of_msolg/ns2_no6_optim_img.jpg")
    # m_img = imread("final_data/optical-lidar-3/ssim_of_msolg/ns2_no6_optim_img.jpg")[:, :, 0]

    print(f_img.shape)
    print(m_img.shape)

    m_img[np.isnan(m_img)] = 0

    f_img = np.expand_dims(np.expand_dims(f_img, axis=0), axis=0)
    m_img = np.expand_dims(np.expand_dims(m_img, axis=0), axis=0)

    mind = MIND()
    f_img = torch.tensor(f_img, dtype=torch.float32)
    f_mind = mind(f_img)
    m_img = torch.tensor(m_img, dtype=torch.float32)
    m_mind = mind(m_img)

    # print(f_mind.cpu().detach().numpy().shape)
    #
    loss = MINDLoss()
    print(loss(f_img, m_img).cpu().detach().numpy())

    mind_val = torch.mean(torch.abs(f_mind-m_mind))

    print(mind_val.cpu().detach().numpy())
