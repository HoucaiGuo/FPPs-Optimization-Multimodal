...
if __name__ == "__main__":
    for test in ["optical-infrared-1", "optical-lidar-1", "optical-sar-1"]:
        f_img_path = f"parameter_study_all/{test}/f_img.tif"
        m_img_path = f"parameter_study_all/{test}/m_img.tif"
        f_inliers_path = f"parameter_study_all/{test}/f_inliers.csv"
        m_inliers_path = f"parameter_study_all/{test}/m_inliers.csv"
        ...
        for ns in range(1, 2):
            for no in range(4, 9):
                print(f"test: {test}, nscale: {ns}, norient: {no}")
                ...
                optim_img_path = f"parameter_study_all/{test}/ns{ns}-no{no}/ns{ns}_no{no}_optim_img.jpg"
                losses_path = f"parameter_study_all/{test}/ns{ns}-no{no}/ns{ns}_no{no}_losses.csv"
                # region points optimization, image registration
                plt_optim = PointsOptimization(f_img, m_img, f_inliers, m_inliers, criterion="ssim_of_msolg",
                                               nscale=ns, norient=no)
                plt_optim.optimize()
                f_optim = plt_optim.f_pts
                m_optim = plt_optim.m_pts
                print(f"Finished the optimization of CPs, {f_optim.shape[0]} remain.")
